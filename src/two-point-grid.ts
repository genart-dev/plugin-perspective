...
const TWO_POINT_PROPERTIES: LayerPropertySchema[] = [
  {
    key: "leftVP",
    label: "Left VP",
    type: "point",
...
/** Fan lines from a VP across the canvas, spread evenly across the opposite edge. */
function fanFromVP(
  vpX: number,
  vpY: number,
  bounds: LayerBounds,
  ...
  // Fan to all 4 edges for full coverage
...
export const twoPointGridLayerType: LayerTypeDefinition = {
  ...
  render(
    ...
    const linesPerVP = (properties.linesPerVP as number) ?? 10;
    const vLines = (properties.verticalLines as number) ?? 8;
    ...
    // Vertical parallel lines (no convergence in 2-point)
    if (vLines > 0) {
      if (useDepthStyling) {
        ctx.globalAlpha = 1;
        ctx.lineWidth = lineWidth;
